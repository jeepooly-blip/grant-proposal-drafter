...
# Try to get API key from secrets (cloud) first, fallback to environment variable
GROQ_API_KEY = st.secrets.get("GROQ_API_KEY", os.getenv("GROQ_API_KEY", ""))

if not GROQ_API_KEY:
    st.error("⚠️ API key not configured. Please add your Groq API key in the Streamlit Cloud Secrets.")
    st.stop()
...
# =============================================================================
# GRANT PROPOSAL DRAFTER - ENHANCED LAYOUT WITH GROQ
# =============================================================================
...
# =============================================================================
# CONFIGURE GROQ AS OPENAI-COMPATIBLE API
# =============================================================================
...
def run_crew(pdf_text: str, company_description: str, word_count: int, tone: str, focus_areas: list) -> str:
    ...
    rfp_analyst = Agent(
        ...
        goal="Extract the core Problem Statement and all explicit Requirements from the RFP.",
    ...
    extract_task = Task(
        description=(
            ...
            "3. Funder's buzzwords and priorities"
        ),
        expected_output="Problem Statement, Key Requirements, and Funder Buzzwords in markdown format.",
        agent=rfp_analyst,
    )

    write_task = Task(
        description=(
            f"Write a professional {word_count}-word Executive Summary.\n\n"
            f"Company Description:\n{company_description}\n\n"
            f"Tone: {tone}\n"
            f"Focus Areas: {', '.join(focus_areas)}\n\n"
            "Structure:\n"
            "- Hook referencing the problem\n"
            "- Organization introduction\n"
            "- Solution addressing requirements\n"
            "- Expected impact (include specific metrics)\n"
            "- Call to action"
        ),
...
with col1:
    ...
    uploaded_file = st.file_uploader(
        ...
        type=["pdf", "txt"],
        help="Upload the Request for Proposals (RFP) document",
...
with col2:
    ...
    company_desc = st.text_area(
        ...
        placeholder="""Example:
Organization Name: Coastal Resilience Alliance (CRA)
...
if not GROQ_API_KEY or not GROQ_API_KEY.startswith("gsk_"):
    st.error("⚠️ Groq API key not found. Get one at https://console.groq.com/keys")
    st.info("Add to .env file: GROQ_API_KEY=gsk_your_key_here")
    st.stop()
else:
